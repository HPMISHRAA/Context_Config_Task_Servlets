...
@WebServlet(urlPatterns = "/normal", initParams = @WebInitParam(name = "discount", value = "15"))
public class NormalBuy extends HttpServlet {
	...
	private void printLogic(HttpServletResponse resp, double price, double discount, String product)
			throws IOException {
		resp.getWriter().print("<h1 align='center' color='red'>Product is " + product.toUpperCase() + "</h1>");
		resp.getWriter().print("<h2 align='center'>Cost Price is : " + price + "&#8377</h2>");
		resp.getWriter().print(
				"<h2 align='center'>Discount is " + discount + "% ie, " + (price * discount) / 100 + "&#8377</h2>");
		resp.getWriter().print("<h2 align='center'>Selling Price is " + (price - (price * discount) / 100) + "</h2>");
	}
}